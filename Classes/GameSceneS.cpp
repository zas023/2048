...
bool GameSceneS::init()
{
	...
	touchListener->onTouchBegan = CC_CALLBACK_2(GameSceneS::onTouchBegan, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(GameSceneS::onTouchEnded, this);
	...
	m_highScore = UserDefault::getInstance()->getIntegerForKey("HIGHSCORE_S", 0);
...
void GameSceneS::getCardSize(Size size){
...
bool GameSceneS::onTouchBegan(Touch* touch, Event* event)
{
	...
	if(spriteFresh->getBoundingBox().containsPoint(beginPoint)){
		...
		auto actionOne =Sequence::create(  
			...
			CallFunc::create(this,callfunc_selector(GameSceneS::startNewGame)),  
			NULL  
			); 
...
//创建生成随机卡片
void GameSceneS::createCardNumber()
{
	//若某方向滑动后不能移动卡片
	if(!is_Move){
		return ;
...
//新游戏
void GameSceneS::startNewGame(){
	if (m_highScore<m_Score)
	{

		m_highScore = m_Score;

		UserDefault::getInstance()->setIntegerForKey("HIGHSCORE_S", m_highScore);
	}
	Director::getInstance()->replaceScene(TransitionFade::create(1, GameSceneS::createScene()));
}

//检测卡片最大值是否到达分数
void GameSceneS::checkWin(){
}

//向左滑动
void GameSceneS::toLeft()
{
	//判断有没有发生移动
	is_Move = false;
	for (int y = 0; y < 4; y++)
...
//更新分数
void GameSceneS::updataScore()
{
	scoreLabel->setString(__String::createWithFormat("Current Score %i",m_Score)->getCString());
}

//监听系统按键
void  GameSceneS::onKeyReleased(EventKeyboard::KeyCode keycode, cocos2d::Event *event){  
